...
class GridState:
    ...
    # 网格层级
    grid_levels: Dict[int, GridLevel] = field(
        default_factory=dict)  # grid_id -> GridLevel
    ...
    # 活跃订单跟踪
    active_orders: Dict[str, GridOrder] = field(
        default_factory=dict)  # order_id -> GridOrder
    ...
    def mark_order_filled(self, order_id: str, filled_price: Decimal, filled_amount: Decimal):
        ...
        # 更新完成循环次数
        self.completed_cycles = min(
            self.filled_buy_count, self.filled_sell_count)
    ...
    def calculate_unrealized_profit(self) -> Decimal:
        ...
        self.unrealized_profit = (
            self.current_price - self.average_cost) * self.current_position
