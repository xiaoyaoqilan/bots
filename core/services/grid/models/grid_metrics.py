...
class GridStatistics:
    ...
    # 余额数据来源：WebSocket缓存 / REST API
    balance_data_source: str = "REST API"
    ...
    # 🔥 循环APR预估（基于运行时间和循环次数）
    cycle_apr_estimate: Decimal = Decimal('0')  # 现有循环APR预估（基于全部运行时间，百分比）
    cycle_profit_percentage: Decimal = Decimal('0')  # 每次循环盈利百分比（扣除手续费后）

    # 🆕 实时循环APR预估（基于过去10分钟）
    realtime_cycle_apr_estimate: Decimal = Decimal(
        '0')  # 实时循环APR预估（基于过去10分钟，百分比）

    # 🔥 APR计算公式数据（用于UI显示）
    cycle_apr_formula_data: Dict = field(default_factory=dict)  # 现有APR计算公式数据
    # 包含字段：
    # - net_profit_per_cycle: 每次循环净收益
    # - cycles_per_hour: 循环频率（次/小时）
    # - cycles_per_year: 年循环次数
    # - annual_profit_amount: 年化收益金额
    # - grid_total_capital: 网格总仓位（本金基准）

    # 🆕 实时APR计算公式数据（用于UI显示）
    realtime_apr_formula_data: Dict = field(
        default_factory=dict)  # 实时APR计算公式数据
    # 包含字段与cycle_apr_formula_data相同
