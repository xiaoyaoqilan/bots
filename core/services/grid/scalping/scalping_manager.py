...
from .smart_scalping_tracker import SmartScalpingTracker
...
class ScalpingManager:
    ...
    def __init__(self, config: GridConfig):
        ...
        # 🆕 智能剥头皮追踪器（如果启用）
        self._smart_tracker: Optional[SmartScalpingTracker] = None
        if config.smart_scalping_enabled:
            self._smart_tracker = SmartScalpingTracker(
                grid_type=config.grid_type,
                grid_count=config.grid_count,
                initial_trigger_grid=self._trigger_grid,
                allowed_deep_drops=config.allowed_deep_drops,
                min_drop_threshold_percent=config.min_drop_threshold_percent
            )
            self.logger.info(
                f"🧠 剥头皮管理器初始化（智能模式）: "
                f"初始触发={config.scalping_trigger_percent}% (第{self._trigger_grid}格), "
                f"允许深度下跌={config.allowed_deep_drops}次, "
                f"最小下跌阈值={config.min_drop_threshold_percent}%, "
    ...
    def should_trigger(self, current_price: Decimal, current_grid_index: int) -> bool:
        ...
        逻辑：
            常规模式：当价格跌到触发点时直接触发
            智能模式：通过SmartScalpingTracker管理状态，支持多次深度下跌追踪
        """
        ...
        # 🆕 智能剥头皮模式
        if self._smart_tracker is not None:
            should_trigger = self._smart_tracker.update(current_grid_index)
            if should_trigger:
                # 打印智能剥头皮激活日志
                progress_info = self._smart_tracker.get_progress_info()
                self.logger.warning(
                    f"🔴 智能剥头皮模式激活! "
                    f"当前价格${current_price:.2f} (Grid {current_grid_index}), "
                    f"完成{progress_info['drop_count']}次深度下跌, "
                    f"激活Grid {progress_info['activation_grid']}"
                )
            return should_trigger

        # 🔥 常规剥头皮模式（原有逻辑）
        is_long = self.config.grid_type in [
        ...
        if is_long:
            ...
            if should_trigger:
                self.logger.warning(
                    f"🔴 触发剥头皮模式 (做多): "
                    f"当前价格${current_price:.2f}, 第{current_grid_index}格 <= 触发点第{self._trigger_grid}格"
                )
            return should_trigger
        else:
    ...
    def get_smart_progress_info(self) -> Optional[dict]:
        """
        获取智能剥头皮进度信息（用于UI显示）
        ...
        """
        if self._smart_tracker is not None:
            return self._smart_tracker.get_progress_info()
        return None

    def is_smart_scalping_enabled(self) -> bool:
        """判断是否启用智能剥头皮模式"""
        return self._smart_tracker is not None
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"smart={self._smart_tracker is not None})"
        )
