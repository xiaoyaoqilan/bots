...
class BalanceMonitor:
    ...
    def __init__(self, engine, config, coordinator, update_interval: int = 60):
        ...
        Args:
            ...
            update_interval: 余额更新间隔（秒）- 🔥 默认60秒（降低REST API频率）
        """
        self.logger = get_logger(__name__)
        self.engine = engine
        self.config = config
        self.coordinator = coordinator
        self._update_interval = update_interval  # 🔥 默认从10秒改为60秒
    ...
    async def update_balance(self):
        ...
        🔥 优化：优先使用 WebSocket 缓存，只有在缓存失效时才调用 REST API
        """
        try:
            # 🔥 优先使用 WebSocket 缓存（避免频繁REST查询）
            if hasattr(self.engine.exchange, '_balance_cache'):
                ...
                if cached_balance:
                    ...
                    if cache_age < 180:
                        ...
                        self._last_data_source = "WebSocket缓存"

                    self.logger.info(
                        f"🔍 [余额] WS缓存: USDC "
                        f"可用=${self._spot_balance:,.2f}, 总额=${self._collateral_balance:,.2f}, "
                        f"缓存年龄={cache_age:.1f}秒"
                    )

                    # 🔥 重要：初始化各个管理器的本金（首次获取时）
                    # 即使使用WebSocket缓存，也需要初始化本金
